...
public class ConfigUtil {
    ...
    public ConfigUtil(String fileName) {
        try {
            file = new File(FrostyEnchants.getInstance().getDataFolder(), fileName); // Creates a new file object.

            if (!file.exists()) {
                file.getParentFile().mkdirs(); // Makes folders / directories if needed.
                FrostyEnchants.getInstance().saveResource(fileName, false); // Saves .yml in folder
            }
            ...
            if (e instanceof InvalidConfigurationException) {
                ...
                if (file.renameTo(new File(FrostyEnchants.getInstance().getDataFolder(), "broken_" + fileName))) { // Tries to rename their file
                    System.out.println("We renamed your configuration to broken_" + fileName);
                    FrostyEnchants.getInstance().saveResource(fileName, false); // Creates a new default configuration for them
